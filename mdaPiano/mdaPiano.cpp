...
mdaPiano::mdaPiano(float samplerate, INT32 buffersize) : buffersize(buffersize), samplerate(samplerate) {
    Fs = samplerate;  iFs = 1.0f / Fs;  cmax = 0x7F;  //just in case...
...
void mdaPiano::setProgram(INT32 program) {
    curProgram = program;
    update();
...
void mdaPiano::setParameter(INT32 index, float value) {
    programs[curProgram].param[index] = value;
    update();
}


void mdaPiano::fillpatch(INT32 p, char* name, float p0, float p1, float p2, float p3, float p4,
...
float mdaPiano::getParameter(INT32 index) { return programs[curProgram].param[index]; }
...
void  mdaPiano::setBlockSize(INT32 blockSize) { buffersize = blockSize; }
...
bool mdaPiano::getProgramNameIndexed(INT32 category, INT32 index, char* text) {
...
bool mdaPiano::copyProgram(INT32 destination) {
...
INT32 mdaPiano::canDo(char* text) {
...
void mdaPiano::getParameterName(INT32 index, char* label) {
    switch (index) {
...
void mdaPiano::getParameterDisplay(INT32 index, char* text) {
    char string[16];
    float* param = programs[curProgram].param;
...
void mdaPiano::getParameterLabel(INT32 index, char* label) {
    switch (index) {
...
void mdaPiano::guiGetDisplay(INT32 index, char* label) {
...
void mdaPiano::processReplacing(float* inputbuf, float* outputbuf, int inchannels, int outchannels, INT32 sampleframes) {
    float* out1 = outputbuf;
    float* out2 = outputbuf + 1;
...
void mdaPiano::noteOn(INT32 note, INT32 velocity) {
    float* param = programs[curProgram].param;
    float l = 99.0f;
    INT32 v, vl = 0, k, s;
...
INT32 mdaPiano::processEvents(std::vector<MidiEvent>& ev) {
    INT32 npos = 0;

    for (INT32 i = 0; i < ev.size(); i++) {
        MidiEvent event = ev[i];
        char* midiData = event.msgBytes;

        switch (midiData[0] & 0xf0) //status byte (all channels)
        {
        case 0x80: //note off
            notes[npos++] = event.sample; //delta
