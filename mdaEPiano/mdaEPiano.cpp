...
mdaEPiano::mdaEPiano(UINT32 samplerate, UINT32 buffersize) : Fs(samplerate), buffersize(buffersize) {
    iFs = 1.0f/Fs;  //just in case...

    programs = new struct mdaEPianoProgram[NPROGS];
    if (programs) {
        //fill patches...
        INT32 i = 0;
...
void mdaEPiano::setProgram(INT32 program) {
	curProgram = program;
    update();
}


void mdaEPiano::setParameter(INT32 index, float value) {
...
void mdaEPiano::fillpatch(INT32 p, char *name, float p0, float p1, float p2, float p3, float p4,
                      float p5, float p6, float p7, float p8, float p9, float p10,float p11) {
    programs[p].name = name;
...
float mdaEPiano::getParameter(INT32 index)     { return programs[curProgram].param[index]; }
void mdaEPiano::setProgramName(char* name) { programs[curProgram].name = name; }
std::string mdaEPiano::getProgramName() { return programs[curProgram].name; }
void mdaEPiano::setBlockSize(INT32 blockSize) { buffersize = blockSize; }
std::string mdaEPiano::getEffectName()    { return "ePiano"; }
std::string mdaEPiano::getVendorString() { return "mda"; }
std::string mdaEPiano::getProductString() { return "mda ePiano"; }


//bool mdaEPiano::getOutputProperties(UINT32 index, VstPinProperties* properties)
//{
//	if(index<NOUTS)
//	{
...
std::string mdaEPiano::getProgramNameIndexed(INT32 category, INT32 index)
{
...
bool mdaEPiano::copyProgram(INT32 destination)
{
...
INT32 mdaEPiano::canDo(char* text) {
...
std::string mdaEPiano::getParameterName(INT32 index) {
	switch (index)
	{
    case  0: return "Env. Decay";
    case  1: return "Env. Release";
    case  2: return "Hardness";
		
    case  3: return "Treb. Boost";
	case  4: return "Modulation";
	case  5: return "LFO Rate";
		
    case  6: return "Vel. Sense";
    case  7: return "Stereo Width";
    case  8: return "Polyphony";

    case  9: return "Fine Tuning";
	case 10: return "Rnd Tuning";
	default: return "Overdrive";
	}
}


std::string mdaEPiano::getParameterDisplay(INT32 index) {
	char string[16];
	float* param = programs[curProgram].param;
...
std::string mdaEPiano::getParameterLabel(INT32 index) {
  switch(index)
  {
   case  5: return "Hz";
   case  8: return "voices";
    case  9: 
    case 10: return "cents";
    default: return "%";
  }
}


void mdaEPiano::processReplacing(float* inbuffer, float* outbuffer, int inputChannels, int outputChannels, INT32 sampleFrames) {
    float* curSample = outbuffer;
...
void mdaEPiano::noteOn(INT32 note, INT32 velocity)
{
  float * param = programs[curProgram].param;
  float l=99.0f;
  INT32  v, vl=0, k, s;
  ...
  if(velocity > 0) 
  {
    ...
    s = size;
    //if(velocity > 40) s += (UINT32)(sizevel * (float)(velocity - 40));  - no velocity to hardness in ePiano
...
UINT32 mdaEPiano::processEvents(std::vector<MidiEvent>& events) {
    float * param = programs[curProgram].param;
    INT32 npos=0;
  
    for (INT32 i=0; i < events.size(); i++) {
        char* midiData = events[i].msgBytes;
        switch(midiData[0] & 0xf0) //status byte (all channels)
        {
            case 0x80: //note off
            notes[npos++] = events[i].sample; //delta
