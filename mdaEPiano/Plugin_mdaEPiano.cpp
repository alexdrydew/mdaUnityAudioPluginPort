...
namespace MDAEPiano {
    ...
    int InternalRegisterEffectDefinition(UnityAudioEffectDefinition& definition) {
        ...
        RegisterParameter(definition, "Treb. boost", "%", 0.0f, 1.0f, 0.5f, 100, 1, P_TBOOST);
        RegisterParameter(definition, "Modulation", "%", 0.0f, 1.0f, 0.5f, 100, 1, P_MODUL);
        RegisterParameter(definition, "LFO rate", "%", 0.0f, 1.0f, 0.5f, 100, 1, P_LFO);
        ...
        RegisterParameter(definition, "Overdrive", "%", 0.0f, 1.0f, 0.5f, 100, 1, P_OVER);
        return numparams;
    }
    ...
    UNITY_AUDIODSP_RESULT UNITY_AUDIODSP_CALLBACK CreateCallback(UnityAudioEffectState* state) {
        ...
        effectdata->plugin = new mdaEPiano(state->samplerate, state->dspbuffersize);
...
extern "C" UNITY_AUDIODSP_EXPORT_API void MDAEPiano_MuteAll() {
    MDAEPiano::MuteAll();
}

extern "C" UNITY_AUDIODSP_EXPORT_API void MDAEPiano_PlayNote(UINT64 sample, UINT32 note, UINT32 velocity, UINT32 sample_length) {
    MDAEPiano::PlayNote(sample, note, velocity, sample_length);
}

extern "C" UNITY_AUDIODSP_EXPORT_API void MDAEPiano_AddEvent(UINT64 sample, UINT32 msg) {
    MDAEPiano::AddEvent(sample, msg);
}
